...
/**************************
*  CVS file version: $Id: new_image.php,v 1.46 2006/06/27 11:20:49 jspurling Exp $
**************************/
...
<div id="caption"><b><?=$admin_lang_new_image?></b></div>
 ...
 ?>
    ...
    <textarea name="body" style="width:97%;height:100px;" rows="" cols=""></textarea><p />
	</div>
    </div>
    <div class='jcaption'>
    <?php echo $admin_lang_ni_datetime; ?></div>
    <div class='content'>
     <input type="radio" name="autodate" value="2" checked="checked" /><?php echo $admin_lang_ni_post_now . ' (~'.gmdate("Y-m-d H:i:s",time()+(3600 * $tz)).')'; ?><br />
     <input type="radio" name="autodate" value="1" /><?php echo $admin_lang_ni_post_one_day_after; ?><br />
     <input type='radio' name='autodate' value='3' id="exifdate" /><?php echo $admin_lang_ni_post_exif_date; ?><br />
     <input type='radio' name='autodate' value='0' id="specificdate" /><?php echo $admin_lang_ni_post_spec_date; ?><br /><br />
 ...
 <tr><td>
  <select name='post_year' onchange="return ondatechange()" >
	<option value='<?php  echo gmdate("Y",time()+(3600 * $tz)); ?>'><?php echo gmdate("Y",time()+(3600 * $tz)); ?></option>
   ...
   // save new post
	...
	if( $_POST['autodate'] == 1 ) {
		...
		if( $row ) $datetime = $row[0];	// If there is none, will default to the other value
			...
			if( $_POST['autodate'] == 2 ) {
			 $datetime = gmdate("Y-m-d H:i:s",time()+(3600 * $tz));} else if ($_POST['autodate'] == 3)// exifdate
