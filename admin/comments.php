...
/**************************
*  CVS file version: $Id: comments.php,v 1.37 2006/06/20 22:40:17 gajcy Exp $
**************************/
...
*/
				if (!isset($_POST['numimg_pp']) && !isset($_SESSION['numimg_pp'])){
					$_SESSION['numimg_pp'] = 10;
				}elseif (isset($_POST['numimg_pp'])){
					$_SESSION['numimg_pp'] = ($pixelpost_commentnumb < $_POST['numimg_pp'])?$pixelpost_commentnumb:$_POST['numimg_pp'];
				};
  ...
  while( $row = mysql_fetch_assoc($images)) {
    ...
    if($pixelpost_commentnumb >$_SESSION['numimg_pp'] ) {
	       ...
	       echo "\n".$image_page_Links."<br>";
	       if (isset($_GET['show']))
	       	$from_html = '<form method="post" action="'.$PHP_SELF .'?view=comments&page=0&amp;show=masked" accept-charset="UTF-8">';
	       	else
	       	$from_html = '<form method="post" action="'.$PHP_SELF .'?view=comments&page=0" accept-charset="UTF-8">';
	       echo $from_html."\n";
